//#include "PBRBloomReflectionRenderPass.h"
//#include "SkyboxMesh.h"
//
//PBRBloomReflectionRenderPass::PBRBloomReflectionRenderPass() : RenderPass()
//{
//}
//
//PBRBloomReflectionRenderPass::~PBRBloomReflectionRenderPass()
//{
//}
//
//void PBRBloomReflectionRenderPass::BuildRenderPass(PBRRenderPassTextureSubmitList& textures)
//{
...
//    {
//        DrawToBloomMap = std::make_shared<RenderedCubeMapTexture>(RenderedCubeMapTexture(RenderPassResolution, VK_FORMAT_R32G32B32A32_SFLOAT, VK_SAMPLE_COUNT_1_BIT));
//        for (unsigned int mip = 0; mip < TextureMapMipLevels; mip++)
//        {
//            BloomMapList.emplace_back(std::make_shared<RenderedCubeMapTexture>(RenderedCubeMapTexture(glm::vec2(static_cast<float>(RenderPassResolution.x * std::pow(0.5f, mip)), static_cast<float>(RenderPassResolution.y * std::pow(0.5f, mip))), VK_FORMAT_R32G32B32A32_SFLOAT, VK_SAMPLE_COUNT_1_BIT)));
//        }
...
//}
//
//void PBRBloomReflectionRenderPass::OneTimeDraw(PBRRenderPassTextureSubmitList& textures)
//{
//    RenderPassResolution = VulkanRenderer::GetSwapChainResolutionVec2();
//    TextureMapMipLevels = static_cast<uint32_t>(std::floor(std::log2(std::max(RenderPassResolution.x, RenderPassResolution.y)))) + 1;
//
...
//            BloomMapList.emplace_back(std::make_shared<RenderedCubeMapTexture>(RenderedCubeMapTexture(glm::vec2(static_cast<float>(RenderPassResolution.x * std::pow(0.5f, mip)), static_cast<float>(RenderPassResolution.y * std::pow(0.5f, mip))), VK_FORMAT_R32G32B32A32_SFLOAT, VK_SAMPLE_COUNT_1_BIT)));
//        }
//        DepthTexture = std::make_shared<RenderedCubeMapTexture>(RenderedCubeMapTexture(RenderPassResolution, VK_SAMPLE_COUNT_1_BIT));
//    }
//    else
//    {
//        DrawToBloomMap->RecreateRendererTexture(RenderPassResolution);
...
//}
//
//void PBRBloomReflectionRenderPass::RenderPassDesc()
//{
...
//}
//
//void PBRBloomReflectionRenderPass::BuildRenderPassPipelines(PBRRenderPassTextureSubmitList& textures)
//{
...
//}
//
//VkCommandBuffer PBRBloomReflectionRenderPass::Draw()
//{
...
//
//        DrawToBloomMap->UpdateCubeMapLayout(commandBuffer, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
//        Texture::CopyMipLevelToCubeMap(commandBuffer, DrawToBloomMap, BloomMapList[mip]);
//        DrawToBloomMap->UpdateCubeMapLayout(commandBuffer, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
//    }
//
//
//    for (unsigned int mip = 0; mip < TextureMapMipLevels; mip++)
...
//}
//
//void PBRBloomReflectionRenderPass::Destroy()
//{
