...
void BlurRenderPass::BuildRenderPass(std::shared_ptr<Texture> texture)
{
    ...
    if (renderPass == nullptr)
    {
        ...
        BlurredTexture = std::make_shared<RenderedColorTexture>(RenderedColorTexture(RenderPassResolution, VK_FORMAT_R32G32B32A32_SFLOAT, VK_SAMPLE_COUNT_1_BIT, TextureMapMipLevels));
    }
    else
    {
        DrawToBloomMap->RecreateRendererTexture(RenderPassResolution);
        BlurredTexture->RecreateRendererTexture(RenderPassResolution);
        RenderPass::Destroy();
    }
    ...
    BuildRenderPassPipelines(texture);
    SetUpCommandBuffers();
}

void BlurRenderPass::OneTimeDraw(std::shared_ptr<Texture> texture)
{
    RenderPassResolution = VulkanRenderer::GetSwapChainResolutionVec2();
    TextureMapMipLevels = static_cast<uint32_t>(std::floor(std::log2(std::max(RenderPassResolution.x, RenderPassResolution.y)))) + 1;
...
void BlurRenderPass::BuildRenderPassPipelines(std::shared_ptr<Texture> texture)
{
    ...
    blurPipeline.InitializePipeline(pipelineInfo, texture);
}

VkCommandBuffer BlurRenderPass::Draw()
{
    if (DrawToBloomMap->GetImageLayout() != VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL &&
        BlurredTexture->GetImageLayout() != VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
    {
        DrawToBloomMap->UpdateImageLayout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        BlurredTexture->UpdateImageLayout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 0);
    }
    ...
    BlurredTexture->UpdateImageLayout(commandBuffer, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 0);
    ...
    {
        for (uint32_t mip = 0; mip < TextureMapMipLevels; mip++)
        {
            VkViewport viewport{};
            ...
            settings.Horizantal = x;
            settings.MipLevel = mip;
            ...
            Texture::CopyTexture(commandBuffer, DrawToBloomMap, BlurredTexture, mip);
            DrawToBloomMap->UpdateImageLayout(commandBuffer, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        }
    }
    BlurredTexture->UpdateImageLayout(commandBuffer, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, 0);
