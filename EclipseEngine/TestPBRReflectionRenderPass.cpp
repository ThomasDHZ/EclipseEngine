...
TestPBRReflectionRenderPass::TestPBRReflectionRenderPass() : RenderPass()
{
}
...
void TestPBRReflectionRenderPass::BuildRenderPass(std::string& renderPass2, PBRRenderPassTextureSubmitList& textures)
{
    ...
    {
        RenderedTexture = std::make_shared<RenderedCubeMapTexture>(RenderedCubeMapTexture(RenderPassResolution, VK_FORMAT_R8G8B8A8_UNORM, VK_SAMPLE_COUNT_1_BIT));
...
void TestPBRReflectionRenderPass::RenderPassDesc()
{
...
void TestPBRReflectionRenderPass::BuildRenderPassPipelines(PBRRenderPassTextureSubmitList& textures)
{
  ...
  /*      LinePipeline = JsonGraphicsPipeline("LinePipeline.txt", LineVertex3D::getBindingDescriptions(), LineVertex3D::getAttributeDescriptions(), renderPass, ColorAttachmentList, VK_SAMPLE_COUNT_8_BIT, sizeof(SceneProperties));
        ...
        PBRPipeline = JsonGraphicsPipeline("PBRReflectionShader.txt", Vertex3D::getBindingDescriptions(), Vertex3D::getAttributeDescriptions(), renderPass, ColorAttachmentList, VK_SAMPLE_COUNT_1_BIT, sizeof(SceneProperties), ReflectionMapSampler, textures);
        SkyBoxPipeline = JsonGraphicsPipeline("SkyBoxSampler.txt", SkyboxVertexLayout::getBindingDescriptions(), SkyboxVertexLayout::getAttributeDescriptions(), renderPass, ColorAttachmentList, VK_SAMPLE_COUNT_1_BIT, sizeof(SkyBoxView), textures.CubeMapTexture);
...
void TestPBRReflectionRenderPass::UpdateView(glm::vec3 reflectPoint)
{
...
VkCommandBuffer TestPBRReflectionRenderPass::Draw(glm::vec3 reflectPoint)
{
    ...
    for (int x = 0; x < GLTFSceneManager::GameObjectList.size(); x++)
    {
        ...
        case GameObjectRenderType::kSpriteRenderer:
        {
            ...
            {
                PBRPipeline.DrawSprite(commandBuffer, GLTFSceneManager::GameObjectList[x], GLTFSceneManager::sceneProperites);
            }
            break;
        }
...
void TestPBRReflectionRenderPass::Destroy()
{
