		...
		for (int y = 0; y < textureAtlasList[0].GetTextureHeightCellCount(); y++)
		{
			for (int x = 0; x < textureAtlasList[0].GetTextureWidthCellCount(); x++)
			{
				for (int layer = 0; layer < 6; layer++)
				{
					...
					vkCmdCopyImage(commandBuffer, textureAtlasList[layer].Image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, reflectionCubeMap->Image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copyImage);
				}
			}
		}
	}
