...
{
	mesh = std::make_shared<Mesh3D>(Mesh3D(VertexList, IndexList, instanceData, MeshSubTypeEnum::kNormal, GameObjectID));
	GameObjectManager::AddGameObject(std::make_shared<MeshRenderer>(*this));
}
...
{
	mesh = std::make_shared<Mesh3D>(Mesh3D(VertexList, IndexList, instanceData, material, MeshSubTypeEnum::kNormal, GameObjectID));
	GameObjectManager::AddGameObject(std::make_shared<MeshRenderer>(*this));
}
