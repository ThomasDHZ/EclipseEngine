...
RayTraceRenderer::RayTraceRenderer(VkDevice Device, VkPhysicalDevice PhysicalDevice, VkCommandPool CommandPool, VkQueue GraphicsQueue, VkDescriptorPool DescriptorPool, uint32_t wIDTH, uint32_t hEIGHT, int swapChainFramebuffersSize, std::vector<VkImage>& swapChainImages)
{
    ...
    camera = std::make_shared<PerspectiveCamera>(PerspectiveCamera("DefaultCamera", VulkanRenderer::GetSwapChainResolutionVec2(), glm::vec3(0.0f, 0.0f, 5.0f)));

    //ModelList.emplace_back(RayTraceModel(device, physicalDevice, commandPool, graphicsQueue, "C:/Users/dotha/source/repos/VulkanGraphics/Models/Sponza/Sponza.obj"));
    ModelList.emplace_back(RayTraceModel(device, physicalDevice, commandPool, graphicsQueue, "C:/Users/dotha/source/repos/VulkanGraphics/Models/viking_room.obj"));
   // ModelList.emplace_back(RayTraceModel(device, physicalDevice, commandPool, graphicsQueue, "C:/Users/dotha/source/repos/VulkanGraphics/Models/Medieval_building.obj"));
  //  ModelList.emplace_back(RayTraceModel(device, physicalDevice, commandPool, graphicsQueue, "C:/Users/dotha/source/repos/VulkanGraphics/Models/vulkanscene_shadow.obj"));
...
void RayTraceRenderer::createBottomLevelAccelerationStructure(RayTraceModel& model, RayTraceMesh& mesh)
{
...
void RayTraceRenderer::createTopLevelAccelerationStructure()
{
    ...
    instancesBuffer.CreateBuffer(AccelerationStructureInstanceList.data(), sizeof(VkAccelerationStructureInstanceKHR) * AccelerationStructureInstanceList.size(), VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
...
void RayTraceRenderer::updateUniformBuffers(GLFWwindow* window)
{
 /*   keyboard.Update(window, camera);
    ...
    ModelList[0].ModelRotation = glm::vec3(0.0f, 0.0f, 180.0f);
   // ModelList[0].ModelRotation = glm::vec3(0.0f, time * 5, 0.0f);
    ...
    SceneDataBuffer.UpdateBufferMemory(&SceneData, sizeof(SceneData));
...
void RayTraceRenderer::createRayTracingPipeline()
{
    ...
    DiffuseBinding.descriptorCount = static_cast<uint32_t>(GLTFSceneManager::TextureList.size());
...
void RayTraceRenderer::createShaderBindingTable() {
    ...
    hitShaderBindingTable.CreateBuffer(shaderHandleStorage.data() + handleSizeAligned * 3, handleSize, VK_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
}
void RayTraceRenderer::createSceneDataBuffer()
{
    SceneDataBuffer.CreateBuffer(&SceneData, sizeof(SceneData), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
}
void RayTraceRenderer::createDescriptorSets()
{
    ...
    UniformBuffer.range = SceneDataBuffer.GetBufferSize();
    ...
    std::vector<RTXMaterial> MaterialList;
    for (int x = 0; x < ModelList.size(); x++)
    {
        for (int y = 0; y < ModelList[x].MeshList.size(); y++)
        {
    ...
    MaterialBuffer.CreateBuffer(MaterialList.data(), sizeof(RTXMaterial) * MaterialList.size(), VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    ...
    for (auto texture : GLTFSceneManager::TextureList)
    {
        VkDescriptorImageInfo DiffuseMapImage = {};
        DiffuseMapImage.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        DiffuseMapImage.imageView = texture->View;
        DiffuseMapImage.sampler = texture->Sampler;
