...
void RayTraceRenderer::createDescriptorPool() {
...
RayTraceRenderer::RayTraceRenderer(uint32_t wIDTH, uint32_t hEIGHT)
{
    device = VulkanRenderer::GetDevice();
    physicalDevice = VulkanRenderer::GetPhysicalDevice();
    commandPool = VulkanRenderer::GetCommandPool();
    graphicsQueue = VulkanRenderer::GetGraphicsQueue();
    WIDTH = wIDTH;
    HEIGHT = hEIGHT;
    ...
    camera.type = RTXCamera::CameraType::lookat;
...
void RayTraceRenderer::buildCommandBuffers()
{
    drawCmdBuffers.resize(VulkanRenderer::GetSwapChainImages().size());
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
    {
        ...
        */
        ...
        barrier.image =  VulkanRenderer::GetSwapChainImages()[i];
        ...
        vkCmdCopyImage(drawCmdBuffers[i], storageImage.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VulkanRenderer::GetSwapChainImages()[i], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copyRegion);
        ...
        barrier3.image = VulkanRenderer::GetSwapChainImages()[i];
