...
void CubeToEnvironmentRenderPass::BuildRenderPass(const glm::vec2& environmentMapSize, std::shared_ptr<RenderedCubeMapTexture> cubeMap)
{
    RenderPassResolution = glm::ivec2(environmentMapSize.x, environmentMapSize.y);
    ...
    {
        environmentMap->RecreateRendererTexture(RenderPassResolution);
        RenderPass::Destroy();
    }

    std::vector<VkImageView> AttachmentList;
    AttachmentList.emplace_back(environmentMap->View);
...
void CubeToEnvironmentRenderPass::RenderPassDesc()
{
    ...
    AttachmentDescriptionList.emplace_back(environmentMap->GetAttachmentDescription());
...
void CubeToEnvironmentRenderPass::BuildRenderPassPipelines(std::shared_ptr<RenderedCubeMapTexture> cubeMap)
{
    ...
    cubeToEnvironmentPipeline.InitializePipeline(pipelineInfo, cubeMap);
}

void CubeToEnvironmentRenderPass::Draw()
{
    ...
    cubeToEnvironmentPipeline.Draw(CommandBuffer[VulkanRenderer::GetCMDIndex()]);
...
void CubeToEnvironmentRenderPass::Destroy()
{
    ...
    cubeToEnvironmentPipeline.Destroy();
    RenderPass::Destroy();
}